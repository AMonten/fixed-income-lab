...
- 162 tests (2026-09-19), ~97% coverage, 0 tests anclados contra un motor externo
  (QuantLib u otro) — sí hay casos de referencia derivados a mano y citados (ver
  #13, #15), pero `tests/reference/` (#37) como infraestructura formal de fixtures
  sigue sin existir.
- Instrumentos: `Bond` (bullet), `ZeroCouponBond`, `FloatingRateNote`,
  `AmortizingBond` (level/explicit/factor-driven).
- Day-count: ACT/360, ACT/365F, 30/360 US, ACT/ACT ICMA (2026-09-18, #15). Falta ACT/ACT ISDA, 30E/360.
- Calendario de días hábiles: `Calendar` inyectable (2026-09-19, #18) —
  `WeekendOnlyCalendar` (default, `weekday() < 5`, sin cambio de comportamiento) y
  `UnitedStatesFederalCalendar` (holidays federales US reales). Falta un calendario
  específico de mercado de fixing (SOFR, etc.) y `ScheduleSpec` (#19).
...
|---|---|---|---|---|
| Implementar `AccrualConvention` / `YieldConvention` / `DiscountCurve` como políticas explícitas e inyectables (ver diseño arriba) | Hoy `t` en `present_value`/YTM sale de `day_count.year_fraction(settlement, payment_date)`, un híbrido que no reproduce ninguna convención de mercado real. Mismo patrón de inyección que `DayCountConvention`. | [#13](https://github.com/AMonten/fixed-income-lab/issues/13) | `test_par_bond_prices_to_exactly_100` (0.1) pasa a verde; `present_value_from_curve` no cambia de resultado en los tests existentes. | hecho — `YieldConvention.time_convention` (`STREET`/`TRUE`, ver `pricing/yield_convention.yield_time_fractions`) separa el conteo de cuasi-cupones del descuento por curva; además se encontró y arregló un segundo defecto de la misma naturaleza en `AccrualConvention`: el cupón de un período *regular* se armaba con `day_count.year_fraction()` (nunca exactamente `1/frecuencia` bajo ACT/365 real) en vez del monto plano de mercado — ver `cashflows/generator._is_regular_period`. `pricing/accrued_interest.py` ahora lee el cupón del cash flow generado en vez de recalcularlo, evitando que las dos lógicas diverjan. No implementa todavía `ScheduleSpec`/`Calendar` (#18/#19) ni el contexto de schedule para ACT/ACT (#14) — eso sigue pendiente. |
| Extender la interfaz `DayCountConvention` para que reciba contexto de schedule, no solo `(start, end)` | ACT/ACT ICMA no es computable a partir de dos fechas aisladas: necesita los períodos de cupón. La firma actual `year_fraction(start, end)` es estructuralmente insuficiente. Cambio transversal: afecta `cashflows/generator.py`, `pricing/accrued_interest.py`, `pricing/present_value.py`, `risk/duration.py`, `risk/convexity.py`. **Bloquea ICMA e ISDA** — sin esta fila, quien tome el ítem de ICMA va a asumir que es "una clase más registrada en el `_REGISTRY`" y se va a chocar con un refactor de seis módulos. | [#14](https://github.com/AMonten/fixed-income-lab/issues/14) | Las convenciones existentes (ACT/360, ACT/365F, 30/360) migran a la nueva firma sin cambiar de resultado en ningún test existente. | hecho — con un alcance más chico que la lista original de archivos "afectados", verificado archivo por archivo: `year_fraction(start, end, context: ScheduleContext \| None = None)` nuevo (`ScheduleContext = reference_period_start/reference_period_end/frequency`, en `conventions/day_count.py`; nombres de campo ajustados durante #15 — ver esa fila: para un stub, el período de referencia no es el propio stub sino el período nominal completo que lo contiene), y las 3 convenciones existentes lo ignoran (test de regresión: mismo resultado con o sin contexto). Efectivamente enhebrado en los dos únicos call sites donde `[start,end]` cae dentro de un solo período de cupón — `cashflows/generator.py` (sizing de cupón de stub) y `instruments/amortizing.py` (interés por período), con test que espía el `ScheduleContext` recibido. **No** enhebrado en `pricing/present_value.py` ni en la rama `TRUE` de `pricing/yield_convention.py`: ahí `[start,end]` va de `settlement_date` a un `payment_date` arbitrario que puede abarcar muchos períodos de cupón — no hay un "período de referencia" único que pasar, y forzar uno sería engañoso, no solo redundante. `pricing/accrued_interest.py` tampoco cambió: ya calculaba la fracción devengada con `day_count.day_count()` (conteo de días crudo), nunca con `year_fraction()`. |
| ACT/ACT ICMA | Las convenciones ACT/ACT basadas en período de cupón, ICMA incluida, son P0: necesarias para soportar correctamente los principales mercados soberanos y de bonos internacionales de tasa fija. *(Corrección: no es específicamente para Treasuries — ver nota abajo.)* Prioridad más alta de las dos convenciones ACT/ACT faltantes. | [#15](https://github.com/AMonten/fixed-income-lab/issues/15) | Caso de referencia de Bloque 5 (bono EUR bajo ICMA) dentro de tolerancia documentada. | hecho — `ActualActualICMA` en `conventions/day_count.py`, registrada como `"ACT/ACT-ICMA"` (`DayCount.ACT_ACT_ICMA`). `year_fraction = días_reales[start,end] / (frecuencia × días_reales_del_período_de_referencia)`; sin `ScheduleContext` levanta `ValueError` (no computable de dos fechas aisladas, por diseño). Al implementarla se encontró que el `ScheduleContext` de #14 pasaba los bounds del propio período (incluido stub) en vez del período de referencia *nominal* — corregido ahí mismo (campos renombrados a `reference_period_start`/`reference_period_end`; `cashflows/generator.py` e `instruments/amortizing.py` ahora derivan el período de referencia con `add_months(accrual_end, -meses_del_período)`, válido porque V1 solo produce stubs cortos al frente). Caso de referencia Bloque 5 (bono EUR semi-anual, cupón stub prorrateado, `tests/test_day_count.py::test_eur_bond_stub_coupon_under_act_act_icma_matches_hand_derived_reference`) derivado a mano (regla ICMA 251), tol=1e-9 — citado inline, no en el formato de fixture formal de `tests/reference/` (#37), que sigue sin existir. |
...
| Calendario de días hábiles configurable | Reemplazar `weekday() < 5` hardcodeado por un `Calendar` inyectable (holidays reales). Es prerequisito de "reconciliación real" contra sistemas externos. | [#18](https://github.com/AMonten/fixed-income-lab/issues/18) | `Calendar` es inyectable y al menos un calendario real (p. ej. US) reemplaza `weekday() < 5` en al menos un test de fecha de pago/reset. | hecho — `conventions/calendar.py` nuevo: `Calendar` (ABC), `WeekendOnlyCalendar` (default, `WEEKEND_ONLY`, comportamiento V1 sin cambios) y `UnitedStatesFederalCalendar` (holidays federales US con regla de *observed* fin de semana, cacheado por año). `is_business_day`/`adjust_business_day`/`subtract_business_days`/`generate_schedule` en `cashflows/schedule.py` reciben `calendar` como parámetro opcional; enhebrado hasta `Bond`, `ZeroCouponBond`, `FloatingRateNote` (incluido `reset_lag_days`, mismo bug que #11/0.8) y `AmortizingBond` (incluido `with_level_principal`). Tests de regresión: `test_generate_schedule_with_calendar_moves_payment_date_off_a_holiday` (payment date) y `test_reset_lag_with_a_real_calendar_skips_a_holiday` (reset date), ambos con 4 de julio 2024 como feriado real que un calendario weekend-only no ve. No incluye `ScheduleSpec` (#19) ni calendario específico de mercado de fixing (SOFR, etc. — sigue abierto, ver nota en #11/#18 original). |
...
| Fix de LSP en `ZeroCouponBond` | Convertir a factory (`Bond.zero_coupon(...)`) o clase hermana bajo un contrato común, no herencia que descarta un parámetro posicional. | [#7](https://github.com/AMonten/fixed-income-lab/issues/7) (mismo bug que 0.4) | Test de 0.4 pasa a verde. | hecho — clase hermana (no factory): `ZeroCouponBond` ya no es subclase de `Bond`, sin `coupon_rate` en el constructor. Sitios que dependían de la herencia (`isinstance(security, Bond)` en `app/streamlit_app.py`, type hints de `analyze_bond`/`compute_accrued_interest`) actualizados a `Bond \| ZeroCouponBond`. El Protocol formal de #20 sigue pendiente. |
